...
System.register(["lodash", "./util.js", "./signer.js", "./describer"], function (_export, _context) {
  "use strict";

  var _, Util, CmsSigner, Describer, _createClass, GenericDatasource;
    ...
    execute: function () {
      ...
      _export("GenericDatasource", GenericDatasource = function () {
        ...
        _createClass(GenericDatasource, [{
            ...
            options.targets.forEach(function (target) {
              ...
              var request = _this.backendSrv.datasourceRequest({
                ...
                if (response.status == '200' && response.data.Code == '200') {
                  return _this._transformQueryResponse(response, xcol, ycol, dimensions, describe).then(function (res) {
          ...
          key: "_transformQueryResponse",
          value: async function _transformQueryResponse(response, xcol, ycol, dimensions, describe) {
            var resResult = [];
            ...
            if (dimensions.includes("instanceId")) {
              var instanceIds = _.uniq(_.map(dataDatapoints, 'instanceId'));
            ...
            ycol.map(function (ycolTarget) {
              if (dimensions.includes("instanceId")) {
                var dims = JSON.parse(decodeURIComponent(dimensions).replace(/;/g, ','));
                var _iteratorNormalCompletion = true;
                var _didIteratorError = false;
                var _iteratorError = undefined;

                try {
                  for (var _iterator = dims[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    var dim = _step.value;
                    ...
                    resResult.push({
                      target: target,
                      datapoints: target_datapoints[dim.instanceId].map(function (p) {
                ...
                var datapoints = [];
                dataDatapoints.forEach(function (point) {
                  var datapoint = [];
